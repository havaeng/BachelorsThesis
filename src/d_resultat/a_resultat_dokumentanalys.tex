...
Vidare spås C-DAS och SFERA bidra till förbättrin-gar inom energibesparing, punktlighet, kapacitet, kraftförsörjning, trafikstörningshantering, trafiksäkerhet och koldioxidutsläpp~\cite{IRS90940}.
\subsubsection{SFERA:s avgränsning}
SFERA sträcker sig bortom datautbyte men avhandlar exempelvis inte presentation av data för lokförare, återgivningen hastighetsrekommendationer eller information för lokförare, lokförarens beteende eller handlingar, algoritmer för beräkning av körprofil och så vidare~\cite{IRS90940}.
...
\paragraph{}
\hspace{-11pt}En journey profile (se tabell~\ref{tab:jp}) består av flera delar; tågfärdens unika referens-ID\footnote{Ett för det innevarande trafikdygnet unikt nummer mellan en och sex siffror.}~och information om tågets fysiska egenskaper (train characteristics, TC).
Se beskrivning av TC i tabell(se tabell~\ref{tab:tc}).
Den innehåller även statisk och dynamisk data om tågfärdens innehåll.
Statisk data benämns segment profile (se tabell~\ref{tab:sp}) och är bland annat trafikplatser, lutningsförhållanden, kurvradier, ID för aktuella bana/banor, hastighetsbegränsningar, referenspunkter för C-DAS-OB:s positioneringssystem), signal- och balisplaceringar och mycket mer.
Dynamisk data är inte en egen datamodell men kan röra sig om information tillfälliga hastighetsnedsättningar eller dåliga adhesionsförhållanden inom ett visst område.
...
\begin{table}[hbt!]
    ...
    \begin{tabular}{|l|}
        ...
        \hline
    \end{tabular}
    \caption{Datamodell, journey profile}
    \label{tab:jp}
\end{table}
Status report (se tabell~\ref{tab:sr}) är sättet på vilket C-DAS-OB kan skicka information till TMS\cite{IRS90940}.
...
\begin{table}
    ...
    \begin{tabular}{|l|}
        ...
        \hline
    \end{tabular}
    \caption{Datamodell, status report}
    \label{tab:sr}
\end{table}
\begin{table}
    \centering
    \begin{tabular}{|l|l|l|}
        ...
        \hline
    \end{tabular}
    \caption{Datamodell, segment profile}
    \label{tab:sp}
\end{table}
\begin{table}
    \centering
    \begin{tabular}{|l|}
        ...
        \hline
    \end{tabular}
    \caption{Datamodell, train characteristics}
    \label{tab:tc}
\end{table}
...
Tävlingsmoment och kompetensprövning riktades istället mot andra grupper samt att gruppernas poängställ-ning offentliggjordes på en topplista.
Genom att gå från individuella tävlingar till grupptäv-lingar tillgodosågs samhörighetsbehovet.
\paragraph{}
